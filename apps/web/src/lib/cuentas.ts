...
import { provinciaPermitida } from '@trucazo/economia';
...
export async function crearCuenta(input: {
  ...
  // Control de jurisdicción: el operador puede bloquear provincias donde no
  // tiene habilitación.
  if (!(await provinciaPermitida(input.province))) {
    return {
      ok: false,
      error: 'Por ahora no operamos en tu provincia',
      campo: 'province',
    };
  }
